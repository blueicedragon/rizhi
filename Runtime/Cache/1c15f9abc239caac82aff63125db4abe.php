...
<div id="i_menu1">
	...
	<div id="i_contents" class="i_nicescroll">
		...
		<form action="./index.php?m=<?php echo (MODULE_NAME); ?>&a=listlog" method="post">
			<div class="search-form-inline bg-color clearfix">

				<label>标题</label>
				<input type="text" class="form-control" placeholder="请输入标题名称" autocomplete="off" value="<?php echo ($map["title"]); ?>" name="title">

				<label>项目</label>
				<select name="projectid" id="projectid" class="form-control">
					<option value="0">请选择项目</option>
					<?php if(is_array($plist)): foreach($plist as $key=>$vo): ?><option <?php if(($projectid) == $vo["id"]): ?>selected="selected"<?php endif; ?> value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["title"]); ?></option><?php endforeach; endif; ?>
				</select>
		...
		<?php if(($list) == ""): ?><div class="i_nodata g-margin">
            ...
            <!-- end 无数据 --><?php endif; ?>
		<?php if(($list) != ""): ?><form id="cAll_form" action="./index.php?m=<?php echo (MODULE_NAME); ?>&a=deletelog" method="post">
				<table class="table g-margin">
					...
					<?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
							<td><?php echo ($k); ?></td>
							<td><a href="./index.php?m=<?php echo (MODULE_NAME); ?>&a=detaillog&id=<?php echo ($vo["id"]); ?>"><?php echo ($vo["title"]); ?></a></td>
							<td style="width: 400px; overflow: hidden; display: block; text-overflow: ellipsis; white-space: nowrap;"><?php echo ($vo["content"]); ?></td>
							<td><?php echo ($vo["pname"]); ?></td>
							<td><?php echo ($vo["cname"]); ?></td>
							<td><?php echo ($vo["worktime"]); ?></td>
							<td title="注册时间：<?php echo (date('Y-m-d H:i:s',$vo["regtime"])); ?>">
								<?php echo (date('Y-m-d',$vo["regtime"])); ?>
							</td>
							<td>
								<a href="./index.php?m=<?php echo (MODULE_NAME); ?>&a=editlog&id=<?php echo ($vo["id"]); ?>">修改</a>
								<a href="./index.php?m=<?php echo (MODULE_NAME); ?>&a=detaillog&id=<?php echo ($vo["id"]); ?>">详情</a>
