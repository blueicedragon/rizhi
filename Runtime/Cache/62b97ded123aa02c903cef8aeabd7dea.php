...
<div id="i_menu1">
	...
	<div id="i_contents" class="i_nicescroll">
		...
		<div class="i_current_column g-margin">
			<span class="i_bold">用户列表</span>
		</div>
		<!-- end 当前栏目 -->

		<form action="./index.php?m=<?php echo (MODULE_NAME); ?>&a=lists" method="post">
			<div class="search-form-inline bg-color clearfix">

				<label>用户名称</label>
				<input type="text" class="form-control" placeholder="请输入用户名称" autocomplete="off" value="<?php echo ($map["title"]); ?>" name="title">
		...
		<?php if(($list) == ""): ?><div class="i_nodata g-margin">
            ...
            <!-- end 无数据 --><?php endif; ?>
		<?php if(($list) != ""): ?><form id="cAll_form" action="./index.php?m=<?php echo (MODULE_NAME); ?>&a=del" method="post">
				<table class="table g-margin">
					...
					<?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
							<td><?php echo ($k); ?></td>
							<td><a href="./index.php?m=<?php echo (MODULE_NAME); ?>&a=detail&id=<?php echo ($vo["id"]); ?>"><?php echo ($vo["name"]); ?></a></td>
							<td><?php echo ($vo["username"]); ?></td>
							<td title="注册时间：<?php echo (date('Y-m-d H:i:s',$vo["regtime"])); ?>">
								<?php echo (date('Y-m-d H:i:s',$vo["regtime"])); ?>
							</td>
							<td>
								<a href="./index.php?m=<?php echo (MODULE_NAME); ?>&a=edit&id=<?php echo ($vo["id"]); ?>">修改</a>
								<a href="./index.php?m=<?php echo (MODULE_NAME); ?>&a=detail&id=<?php echo ($vo["id"]); ?>">详情</a>
								<a href="./index.php?m=<?php echo (MODULE_NAME); ?>&a=delete&id=<?php echo ($vo["id"]); ?>">删除</a>
							</td>
