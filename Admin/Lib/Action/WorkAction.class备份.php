...
class WorkAction extends CommonAction
{
    ...
    public function listtime()
    {
        ...
        if($_SESSION['adminInfo']['adminId']==1){
            $count = $work->query("SELECT COUNT(*) as num FROM (SELECT sum(A.worktime),B.title as pname from 
                                    rz_log_work as A LEFT JOIN rz_project as B on A.projectid=B.id LEFT JOIN 
                                    rz_user as C on A.userid=C.id GROUP BY A.projectid) a");
            //var_dump($count[0]["num"]);exit;
            $p = new Page ($count[0]["num"], $PageNum);
            $list = $work->where($data)
                ->table(C('DB_PREFIX').'log_work A')
                ->join(C('DB_PREFIX').'project B on A.projectid=B.id')
                ->join(C('DB_PREFIX').'user C on A.userid=C.id')
            ...
            $count = $work->query("SELECT COUNT(*) as num FROM (SELECT sum(A.worktime),B.title as pname from 
                                    rz_log_work as A LEFT JOIN rz_project as B on A.projectid=B.id LEFT JOIN 
                                    rz_user as C on A.userid=C.id GROUP BY A.projectid where $data) a");
            //var_dump($count[0]["num"]);exit;
            $p = new Page ($count[0]["num"], $PageNum);
            $list = $work->where($data)
                ->table(C('DB_PREFIX').'log_work A')
